  ...
  const fetchData = async (url: string) => {
    ...
    const detailedPokemon = data.results.map(async (pokemon: { url: string }) => {
      ...
      return{
        ...
        types: pokeData.types.map((type: { type: { name: any; }; }) => type.type.name),
        url: pokemon.url,
        weight: pokeData.weight,
        height: pokeData.height,
        abilityName: pokeData.abilities.find((ability: { is_hidden: boolean; ability: { name: string; }; }) => !ability.is_hidden)?.ability.name,
        abilityUrl: pokeData.abilities.find((ability: { is_hidden: boolean; ability: { url: string; }; }) => !ability.is_hidden)?.ability.url,
    ...
    <main>
      ...
      <div className="flex h-[244px] w-full items-center">
        <nav>
          ...
          <h2 className="text-forloop-text-muted-foreground text-center">Search and find Pokémon</h2>
        </nav>
      </div>
      ...
      <Separator className="w-full h-[1px] my-[0px] bg-forloop-border"></Separator>
        ...
        <div className="box-border h-full w-full px-[140px] pb-[40px]">
          ...
          <div className="grid grid-cols-2 gap-x-[20px] gap-y-[40px] md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 xl:gap-x-[80px]"> {/*Grid for cards*/}
            ...
            {pokemon.map((poke) => (
              ...
              }} key = {poke.id}>{/*Send data to details page*/}
              <Card className="grid grid-cols-1 size-fit ease-in-out duration-300 hover:shadow-xl">                
                    <CardHeader className="p-0 max-h-[224px] max-w-[264]"><img className="bg-forloop-bg-secondary w-full h-full object-cover rounded-t-[10px] rendering-pixelated" src={poke?.image} width={266} height={224} alt="Pokemon Image"/></CardHeader>
                      <div className="flex h-[103px] w-[266px] py-[24px]">
                          <CardContent className="h-full w-full">
                            <CardTitle><h3 className="text-forloop-text-foreground capitalize">{poke?.name}</h3></CardTitle>
                            <CardDescription><h4 className="text-forloop-text-muted-foreground">#{poke?.id.toString().padStart(4, '0')}</h4></CardDescription>
                          </CardContent>                 
                      </div>
                    <CardFooter>
                      <div className="flex gap-[8px]">
                        <Badge className="capitalize">{poke?.types[0]}</Badge>
                        {poke?.types[1] && <Badge className="capitalize">{poke?.types[1]}</Badge>}  {/*This is to only render second type if one exists. can do this way as every pokemon has one or two types*/}
                      </div>
                    </CardFooter>                
              </Card>
            </Link>
          ...
          <div className="flex justify-center gap-4 mt-10">
            <Button asChild>
              <a href="#" onClick={(e) => { e.preventDefault(); if (prevUrl) fetchData(prevUrl); }}><ChevronLeft/>Back</a> 
            </Button>
            <Button asChild>
              <a href="#" onClick={(e) => { e.preventDefault(); if (nextUrl) fetchData(nextUrl); }}>Next<ChevronRight/></a>
            </Button>
          </div>
